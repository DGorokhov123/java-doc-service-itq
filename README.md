...
1. В проекте создается еще один модуль `registry`, с отдельной БД. Туда переносится таблица `approval_registry`.
2. В контроллере создается REST API с описанием интерфейса в модуле `common` 
   - для регистрации документа, 
   - поиска записи реестра по id документа
   - проверки наличия записи
3. В сервисе создается Feign-клиент, наследуется, как и контроллер от интерфейса API
